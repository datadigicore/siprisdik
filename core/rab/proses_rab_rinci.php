...
switch ($process) {
	...
	case 'tambahAkun':
		...
		if ($akun == '524119') {
	        ...
	        $lama_hari    = $_POST['lama_hari'][0];
          	$biaya_akom   = str_replace(".", "", $_POST['biaya_akom'][0]);
    ...
    case 'table_upload':
	    ...
	    $column = array(
	      ...
	      array( 'db' => 'npwp',  'dt' => 3),
	      array( 'db' => 'nip',  'dt' => 4),
	      array( 'db' => 'golongan',  'dt' => 5, 'formatter' => function($d,$row){
	      	if ($d == 1) $gol = 'I';
	      	elseif($d == 2) $gol =  'II';
	      	elseif($d == 3) $gol =  'III';
	      	elseif($d == 4) $gol =  'VI';
	      ...
	      array( 'db' => 'pns',  'dt' => 6, 'formatter' => function($d,$row){
	      	if ($d == 0) {
	      		if ($row[1] == 0) {
	      			$pns = 'N/A';
	      		}else{
